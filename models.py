...
class HiddenGemsFilterWrapper(AlgoBase):
    """
    Un Wrapper qui englobe un algorithme Surprise existant et filtre le Top X% 
    des films les plus populaires en forçant leur prédiction à 0.0.
    """
    def __init__(self, base_algo, exclude_top_pct=0.20, **kwargs):
        AlgoBase.__init__(self, **kwargs)
        
        # 1. On stocke le modèle préconfiguré
        self.base_algo = base_algo
        self.exclude_top_pct = exclude_top_pct
        
        # 2. Précalcul de la popularité en interne (pour être 100% autonome)
        df_ratings = load_ratings(surprise_format=False)
        movie_counts = df_ratings[C.ITEM_ID_COL].value_counts()
        
        # Création du dictionnaire {movie_id: rank} (1 = le plus populaire)
        self.item_to_rank = movie_counts.rank(ascending=False, method='first').to_dict()
        
        # Calcul du seuil (ex: si 10 000 films et 20%, le seuil est le rang 2000)
        self.popularity_threshold = len(movie_counts) * self.exclude_top_pct

    def fit(self, trainset):
        # On délègue complètement l'entraînement au modèle d'origine
        self.base_algo.fit(trainset)
        return self

    def test(self, testset, verbose=False):
        # 1. On laisse le vrai modèle faire ses prédictions normales
        raw_predictions = self.base_algo.test(testset, verbose)
        
        filtered_predictions = []
        
        # 2. Le sabotage ciblé : on écrase la note des blockbusters
        for pred in raw_predictions:
            # On cherche le rang du film (s'il n'existe pas, on lui donne le pire rang pour l'accepter)
            rank = self.item_to_rank.get(pred.iid, float('inf'))
            
            if rank <= self.popularity_threshold:
                # Le film est trop populaire ! On écrase sa note pour qu'il n'apparaisse jamais dans le Top-N
                new_est = 0.0
            else:
                # C'est une pépite, on garde la vraie note calculée par ton modèle
                new_est = pred.est
                
            # On reconstruit la prédiction pour que le framework Surprise n'y voie que du feu
            filtered_predictions.append(
                Prediction(pred.uid, pred.iid, pred.r_ui, new_est, pred.details)
            )
